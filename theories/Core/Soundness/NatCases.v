...
Proof.
  ...
  destruct (per_univ_of_instance HΓ ltac:(eassumption) ltac:(eassumption) ρ d{{{ zero }}}
...
Lemma per_univ_nat_step_instance : forall {i Γ SbΓ A Sb Δ σ ρ p am},
    ...
    {{ Δ ⊢s σ ® ρ ∈ Sb }} ->
    {{ ⟦ A[Wk ⨟ Wk,,succ #1] ⟧ ρ ↘ p }} ->
    {{ ⟦ A ⟧ ^(drop_env (drop_env ρ)) ↦ succ ^(drop_env ρ 0) ↘ am }} ->
    {{ Dom p ≈ am ∈ per_univ i }}.
Proof.
  intros.
  assert (exists env_relΓ, {{ EF Γ ≈ Γ ∈ per_ctx_env ↘ env_relΓ }}) as [env_relΓ HΓ] by mauto 3.
...
Proof.
  ...
  exact (@cons_glu_sub_pred_q_helper Γ SbΓ Δ σ ρ i {{{ ℕ }}} d{{{ ℕ }}}
...
Proof.
  ...
  assert {{ Γ, ℕ, A ⊢ MS : A[Wk ⨟ Wk,,succ #1] }} by mauto 2.
  invert_glu_rel_exp HMS.
  assert (glu_nat Δ M d{{{ ⇑ a m }}}) by (econstructor; eassumption).
  assert {{ Δ ⊢s σ,,M ® ρ ↦ ⇑ a m ∈ SbΓℕ }} by (unfold SbΓℕ; mauto 3).
  assert (glu_nat Δ {{{ zero }}} d{{{ zero }}}) by (econstructor; mauto 3).
  assert {{ Δ ⊢s σ,,zero ® ρ ↦ zero ∈ SbΓℕ }} by (unfold SbΓℕ; mauto 3).
  destruct_glu_rel_exp_with_sub.
  simplify_evals.
  match_by_head glu_univ_elem ltac:(fun H => directed invert_glu_univ_elem H).
  apply_predicate_equivalence.
  ...
  match goal with
  | _: {{ ⟦ A ⟧ ρ ↦ zero ↘ ^?v }} |- _ => rename v into azero
  end.
  ...
  enough {{ Δ ⊢ rec M return A[q σ] | zero -> MZ[σ] | succ -> MS[q (q σ)] end
             : A[σ,,M] ® rec m under ρ return A | zero -> mz | succ -> MS end ∈ glu_elem_bot i am }}
  ...
  - (** [per_bot_natrec_diag] replaces the hand-rolled readback-existence
    ...
    assert {{ DF mz ≈ mz ∈ per_head A A d{{{ ρ ↦ zero }}} ↘ d{{{ ρ ↦ zero }}} }}
  ...
  - intros Δ' φ W ? HW.
    ...
    assert {{ Δ', ℕ ⊢s q ^(sb_wk σ φ) ® ρ ↦ ⇑! ℕ (length Δ') ∈ SbΓℕ }} by (unfold SbΓℕ; mauto 3).
    destruct_glu_rel_exp_with_sub.
    simplify_evals.
    match_by_head glu_univ_elem ltac:(fun H => directed invert_glu_univ_elem H).
    apply_predicate_equivalence.
    ...
    assert {{ Δ', ℕ, A[q ^(sb_wk σ φ)] ⊢s q (q ^(sb_wk σ φ))
                ® ρ ↦ ⇑! ℕ (length Δ') ↦ ⇑! m0 (S (length Δ')) ∈ SbΓℕA }}
