...
Lemma per_univ_elem_nat : forall i,
    {{ DF ℕ ≈ ℕ ∈ per_univ_elem i ↘ per_nat }}.
Proof.
...
Lemma per_bot_natrec : forall {i Aa Ab MZa MZb MSa MSb ρa ρb za zb m m'}
    ...
    {{ ⟦ MZa ⟧ ρa ↘ za }} ->
    {{ ⟦ MZb ⟧ ρb ↘ zb }} ->
    {{ Dom za ≈ zb ∈ Rel d{{{ zero }}} d{{{ zero }}} }} ->
    (forall w z r r',
        {{ Dom w ≈ z ∈ per_nat }} ->
        {{ Dom r ≈ r' ∈ Rel w z }} ->
        exists s s',
          {{ ⟦ MSa ⟧ ρa ↦ w ↦ r ↘ s }} /\ {{ ⟦ MSb ⟧ ρb ↦ z ↦ r' ↘ s' }} /\
            {{ Dom s ≈ s' ∈ Rel d{{{ succ w }}} d{{{ succ z }}} }}) ->
    {{ Dom m ≈ m' ∈ per_bot }} ->
    {{ Dom rec m under ρa return Aa | zero -> za | succ -> MSa end
         ≈ rec m' under ρb return Ab | zero -> zb | succ -> MSb end ∈ per_bot }}.
Proof.
  intros * Hmot Hza Hzb Hz Hsucc Hm s.
  assert (Hvar : {{ Dom ⇑! ℕ s ≈ ⇑! ℕ s ∈ per_nat }}) by mauto.
  assert (Hzz : {{ Dom zero ≈ zero ∈ per_nat }}) by econstructor.
...
Proof.
  ...
  apply (mk_rel_exp d{{{ succ m1 }}} d{{{ succ m2 }}} d{{{ succ m3 }}} d{{{ succ m4 }}});
...
Proof.
  ...
  apply (mk_rel_exp d{{{ ^(ρσ 1) }}} d{{{ ^(ρσ 1) }}} d{{{ ^(ρ'σ' 1) }}} d{{{ ^(ρ'σ' 1) }}});
...
Proof.
  ...
  assert (CbigR : rel_chain
                    (per_univ_elem i (per_head A A d{{{ ρσ ↦ w }}} d{{{ ρ'σ' ↦ z }}}))
...
Lemma rel_typ_of_nat_step_gen : forall {Γ A i env_relΓ},
    ...
    forall ρ1 ρ2,
      ...
      exists p1 p2 p3 p4,
        ...
        {{ ⟦ A ⟧ ^(drop_env (drop_env ρ1)) ↦ succ ^(drop_env ρ1 0) ↘ p2 }} /\
        {{ ⟦ A ⟧ ^(drop_env (drop_env ρ2)) ↦ succ ^(drop_env ρ2 0) ↘ p3 }} /\
...
Proof.
  ...
  assert (Hτ : forall ρ, {{ ⟦ Wk ⨟ Wk ,, succ #1 ⟧s ρ
                              ↘ ^(drop_env (drop_env ρ)) ↦ succ ^(drop_env ρ 0) }}).
  {
    intros ρ.
...
Proof.
  intros * HΓ HA *.
  exact (rel_typ_of_nat_step_gen HΓ HA d{{{ ρ1 ↦ x1 ↦ y1 }}} d{{{ ρ2 ↦ x2 ↦ y2 }}}).
Qed.
...
Lemma rel_exp_of_nat_step : forall {Γ A i MS MS' env_relΓ},
    ...
    forall Γ' env_rel',
      ...
      forall σ σ' ρ ρ' ρσ ρ'σ',
        ...
        forall w z r r',
          {{ Dom w ≈ z ∈ per_nat }} ->
          {{ Dom r ≈ r' ∈ per_head A A d{{{ ρσ ↦ w }}} d{{{ ρ'σ' ↦ z }}} }} ->
          (exists m m',
              {{ ⟦ MS[q (q σ)] ⟧ ρ ↦ w ↦ r ↘ m }} /\ {{ ⟦ MS ⟧ ρσ ↦ z ↦ r' ↘ m' }} /\
                {{ Dom m ≈ m' ∈ per_head A A d{{{ ρσ ↦ succ w }}} d{{{ ρ'σ' ↦ succ z }}} }}) /\
          (exists m m',
...
Proof.
  ...
  assert (Hrr : {{ Dom r ≈ r ∈ per_head A A d{{{ ρσ ↦ w }}} d{{{ ρ'σ' ↦ z }}} }})
    by (transitivity r'; [| symmetry]; exact Hrr').
  assert (Hr'r' : {{ Dom r' ≈ r' ∈ per_head A A d{{{ ρσ ↦ w }}} d{{{ ρ'σ' ↦ z }}} }})
    by (transitivity r; [symmetry |]; exact Hrr').
  assert (Hr'r : {{ Dom r' ≈ r ∈ per_head A A d{{{ ρσ ↦ w }}} d{{{ ρ'σ' ↦ z }}} }})
  ...
  assert (Hlink : forall ρ1 ρ2 c c',
             rel_chain (per_env_extend {{{ ℕ }}} {{{ ℕ }}} env_relΓ)
               [d{{{ ρσ ↦ w }}}; d{{{ ρ'σ' ↦ z }}}; ρ1; ρ2] ->
             {{ Dom c ≈ c' ∈ per_head A A d{{{ ρσ ↦ w }}} d{{{ ρ'σ' ↦ z }}} }} ->
             {{ Dom ρ1 ↦ c ≈ ρ2 ↦ c'
                  ∈ per_env_extend A A (per_env_extend {{{ ℕ }}} {{{ ℕ }}} env_relΓ) }})
  ...
  assert (ChE : rel_chain (per_env_extend A A (per_env_extend {{{ ℕ }}} {{{ ℕ }}} env_relΓ))
                  [d{{{ ρσ ↦ w ↦ r }}}; d{{{ ρ'σ' ↦ z ↦ r' }}}; d{{{ s ↦ w ↦ r }}};
                   d{{{ s' ↦ z ↦ r' }}}; d{{{ s ↦ w ↦ r' }}}; d{{{ ρσ ↦ z ↦ r' }}};
                   d{{{ ρ'σ' ↦ w ↦ r }}}; d{{{ s' ↦ z ↦ r }}}]).
  ...
  - exists m1, t2.
    ...
    transitivity m2.
    + apply (Hmv d{{{ s ↦ w ↦ r }}} d{{{ s' ↦ z ↦ r' }}}); [ solve_rel_chain | exact Hm12 ].
    + apply (Hmv d{{{ s ↦ w ↦ r' }}} d{{{ ρσ ↦ z ↦ r' }}}); [ solve_rel_chain | exact Ht12 ].
  - exists u1, u2.
    do 2 (split; [ eassumption |]).
    apply (Hmv d{{{ ρσ ↦ w ↦ r }}} d{{{ ρ'σ' ↦ z ↦ r' }}}); [ solve_rel_chain | exact Hu12 ].
  - exists v1, m4.
    do 2 (split; [ eassumption |]).
    transitivity m3.
    + apply (Hmv d{{{ ρ'σ' ↦ w ↦ r }}} d{{{ s' ↦ z ↦ r }}}); [ solve_rel_chain | exact Hv12 ].
    + apply (Hmv d{{{ s ↦ w ↦ r }}} d{{{ s' ↦ z ↦ r' }}}); [ solve_rel_chain | exact Hm34 ].
Qed.
...
Lemma per_bot_natrec_diag : forall {Γ A i MZ MS env_relΓ ρ mz m},
    ...
    {{ Dom ρ ≈ ρ ∈ env_relΓ }} ->
    {{ ⟦ MZ ⟧ ρ ↘ mz }} ->
    {{ Dom mz ≈ mz ∈ per_head A A d{{{ ρ ↦ zero }}} d{{{ ρ ↦ zero }}} }} ->
    {{ Dom m ≈ m ∈ per_bot }} ->
    {{ Dom rec m under ρ return A | zero -> mz | succ -> MS end
         ≈ rec m under ρ return A | zero -> mz | succ -> MS end ∈ per_bot }}.
Proof.
  ...
  eapply (per_bot_natrec (Rel := fun x y => per_head A A d{{{ ρ ↦ x }}} d{{{ ρ ↦ y }}}));
...
Proof.
  ...
  assert (Hz23 : {{ Dom z2 ≈ z3 ∈ per_head A A d{{{ ρσ ↦ zero }}} d{{{ ρ'σ' ↦ zero }}} }})
    by pairwise.
  assert (Hz34 : {{ Dom z3 ≈ z4 ∈ per_head A A d{{{ ρσ ↦ zero }}} d{{{ ρ'σ' ↦ zero }}} }})
    by pairwise.
  ...
  destruct (per_nat_natrec
              (Rel := fun x y => per_head A A d{{{ ρσ ↦ x }}} d{{{ ρ'σ' ↦ y }}})
...
Proof.
  ...
  pose proof (rel_exp_of_nat_step HΓ HA HMS _ _ HΓ' _ _ _ _ _ _ Hσj Hρ Hev Hev')
    as Hstepgen.
  assert (Hc12 : {{ Dom c1 ≈ c2 ∈ per_head A A d{{{ ρσ ↦ m1 }}} d{{{ ρ'σ' ↦ m2 }}} }})
    by (apply (per_head_of_args Hcodn m2 m3 m1 m2 Hm23 Hm12 Hm13); pairwise).
  assert (Hc13 : {{ Dom c1 ≈ c3 ∈ per_head A A d{{{ ρσ ↦ m1 }}} d{{{ ρ'σ' ↦ m3 }}} }})
  ...
  assert (H12 : {{ Dom b1 ≈ b2
                     ∈ per_head A A d{{{ ρσ ↦ succ m2 }}} d{{{ ρ'σ' ↦ succ m3 }}} }})
    by (apply (per_head_of_args Hcodn d{{{ succ m1 }}} d{{{ succ m2 }}}
                 d{{{ succ m2 }}} d{{{ succ m3 }}}
                 (per_nat_succ Hm12) (per_nat_succ Hm23) (per_nat_succ Hm22));
        exact Hb12).
  assert (Hg : {{ Dom g1 ≈ g2
                    ∈ per_head A A d{{{ ρσ ↦ succ m2 }}} d{{{ ρ'σ' ↦ succ m3 }}} }})
    by (apply (per_head_of_args Hcodn d{{{ succ m1 }}} d{{{ succ m3 }}}
                 d{{{ succ m2 }}} d{{{ succ m3 }}}
